...
export class ZeebeJobThrowErrorDialog extends LitElement {
    ...
    render() {
        return html`
            <vaadin-dialog id="job-throw-error-dialog" header-title="Throw error for job" .opened=${this._opened}
                           @opened-changed=${(e) => {this._opened = e.detail.value;}}
                           ${dialogRenderer(() => this._render(), [this._variables, this._errorMessage, this._errorCode, this._item])}
    ...
    _render() {
        return html`
            <vaadin-vertical-layout style="align-items: stretch; width:100%; min-width: 400px; min-height: 200px; max-height: 600px;">
                ...
                <vaadin-text-field label="Error code"
                                   value="${this._errorCode}"
                                   @value-changed=${(e) => {this._errorCode = e.detail.value;}}>
                </vaadin-text-field>                
                <vaadin-text-area
    ...
    _footer = () => html`
        ...
        <vaadin-button theme="primary" @click=${this._action}>Throw error job</vaadin-button>
    `;
    ...
    _action() {
        ...
        this.jsonRpc.jobThrowError({key: this._item.record.key, errorCode: this._errorCode, errorMessage: this._errorMessage, variables: variables})
            .then(response => {
            ...
            .catch(e => {
                console.log(e);
                notifier.showErrorMessage('Throw error for job error: ' + e.error.code + ' detail: ' + e.error.message, null);
            });
    }

}

customElements.define('zeebe-job-throw-error-dialog', ZeebeJobThrowErrorDialog);