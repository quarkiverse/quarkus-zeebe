...
import static io.quarkiverse.zeebe.it.opentracing.TestResources.JAEGER_HOST;
import static io.quarkiverse.zeebe.it.opentracing.TestResources.JAEGER_PORT;
...
public class OpentracingTest {
    ...
    public void testOpenTelemetry() throws JsonProcessingException {
        ...
        JaegerSpan testMethod = spans.get("openTracingTestMethod");
        ...
        assertTag(testTags, "bpmn-class", "io.quarkiverse.zeebe.it.opentracing.OpenTracingTestJobWorker");
    ...
    private JsonPath jaegerTrace() {
        return given().baseUri(JAEGER_HOST).port(JAEGER_PORT)
                .when()
                .get("/api/traces?service=quarkus-zeebe-integration-tests-opentracing&limit=40&lookback=1h")
                .then()
